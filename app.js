...
function filterPrice(event) {
    const responseFilter = event.target.value === 'Menor a 55000'
    ? products.filter(element => element.price <= 55000)
    : event.target.value === '55000 - 90000'
    ? products.filter(element => element.price >= 55000 && element.price <= 90000)
    : event.target.value === 'Mayor a 100000'
    ? products.filter(element => element.price >= 100000)
    : null;
...
function filterCategory(event) {
  const categoria = event.target.value === "francia"
  ? products.filter(element => element.categoria === "francia")
  : event.target.value === 'españa'
  ? products.filter(element => element.categoria === "españa")
  : event.target.value === "paris"
  ? products.filter(element => element.categoria === "paris")
  : event.target.value === "andorra"
  ? products.filter(element => element.categoria === "andorra")
  : event.target.value === "todas"
  ? products.filter(element => element.plataforma === "todas")
  : null;

  main.innerHTML = ''
  categoria.map(element => createProducts(element))
}
...
function createNewProducts() {
    ...
    const newelement = {id:id, name:titleProduct, price:priceProduct, img:imgSelected};
...
function createProducts(products) {
    const {name, img, id, hora, price} = products;
    ...
    datecard.textContent = hora;
    datecard.classList.add('date-product');
    ...
    btnCard.textContent = 'Añadir carrito'
...
const show_games_cart = () => {
  ...
  lista.forEach(item => {
      ...
      info.textContent=todos_productos[0].hora;
